...
class MetaImageSPECT(MetaImageBase):
    ...
    def _init_required_metadata(self, **kwargs):
        ...
        if kwargs["unit"] not in self.authorized_units:
            fatal(
                f'Unit for MetaImageSPECT must be in {self.authorized_units}, while it is {kwargs["unit"]}'
            )
...
class PVCIterativeYang:

    def __init__(self):
        # user param
        self.input_image = None
        self.mask = None
        self.fwhm_mm = [10.0, 10.0, 10.0]
    ...
    def check_param(self):
        ...
        if not isinstance(self.input_image, sitk.Image):
            fatal(f"{cn}: input_image image must be a sitk image, "
                  f"while it is {type(self.input_image)}")
        if not isinstance(self.mask, sitk.Image):
            fatal(f"{cn}: mask image must be a sitk image, "
                  f"while it is {type(self.mask)}")

    def initialize(self):
        self.fwhm_mm = np.array(self.fwhm_mm)
        self.var_mm = np.power(self.fwhm_mm / (2 * np.sqrt(2 * np.log(2))), 2)

    def run(self):
        self.check_param()
        self.initialize()

        # step1: fuzzy corr filter (LATER)

        # convert to numpy
        self.img_arr = sitk.GetArrayViewFromImage(self.input_image)
        self.mask_arr = sitk.GetArrayViewFromImage(self.mask)

        # names in PETPVC Thomas2016:
        # imageEstimate = f_k
        # imageYang = s_k
        self.f_k = np.copy(self.img_arr)  # imageEstimate
        self.s_k = np.copy(self.img_arr)

        # filter
        self.blur_filter = sitk.DiscreteGaussianImageFilter()
        self.blur_filter.SetVariance(self.var_mm)
        self.blur_filter.SetUseImageSpacing(True)
        ...
        # convert to image
        self.f_k[self.mask_arr != 1] = 0  # needed or not ?
        self.output = sitk.GetImageFromArray(self.f_k)
        self.output.CopyInformation(self.input_image)

    def run_iteration(self, i):
        # shorter coding
        m = self.mask_arr

        # step1: mean in the region
        mean = self.f_k[m == 1].mean()
        if self.verbose:
            print(f"Iteration: {i}, mean = {mean}")

        # step2: apply fuzziness correction to
        # current mean value estimates (LATER)
        ...
        if i == 0:
            ...
            self.s_k[m == 1] = self.s_k[m == 1] + mean

        # step4: blur the current s_k
        s_k_img = sitk.GetImageFromArray(self.s_k)
        s_k_img.CopyInformation(self.input_image)
        s_k_img = self.blur_filter.Execute(s_k_img)
        s_k_h = sitk.GetArrayViewFromImage(s_k_img)

        # step5: normalise s_k / blur_s_k (avoid division by zero)
        n_sk = np.divide(
            self.s_k[m == 1],
            s_k_h[m == 1],
            out=np.zeros_like(self.s_k[m == 1]),
            where=(s_k_h[m == 1] != 0),
        )

        # step6: final estimate
        self.f_k[m == 1] = self.img_arr[m == 1] * n_sk
