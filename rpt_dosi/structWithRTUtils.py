...
def image_to_dicom_rt_struct(dicom, mask, output):
    ...
    elif len(series[seriesInstanceUID]) == 1 and series[seriesInstanceUID][0].endswith(
        ".dcm"
    ):
    ...
    for maskFile in tqdm.tqdm(maskFiles):
        ...
        rtstruct.add_roi(
            mask=segArray, color=[255, 0, 255], name=name, approximate_contours=False
        )
...
@click.option(
    "--output",
    "-o",
    help="Output dicom RTStruct filename",
    type=click.Path(
        dir_okay=False,
        writable=True,
        readable=False,
