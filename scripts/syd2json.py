...
def find_for_injection(db, type, patient, dataJSON):
    ...
    injections = syd.find(
        ...
        radionuclide_id=radionuclide_id,
        cycle=cycle,
    )
    ...
    for injection in injections:
        ...
        if "cycle" in type:
            images_NM = syd.find(
                db["Image"], injection_id=injection.id, modality=modality
            )
            ...
            image_NM = syd.find_one(
                db["Image"], injection_id=injection.id, modality=modality
            )
            image_CT = syd.find_one(
                db["Image"], injection_id=injection.id, modality="CT"
            )
